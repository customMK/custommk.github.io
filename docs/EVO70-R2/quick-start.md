...
EVO70 R2 includes a few hex keys/allen wrenches. While you can certainly use them, I don't actually recommend it. It is best to think of them like those small convenience jacks that come with a car: it can be useful in an emergency if it's all you've got, but if you've got better tools, you really want to use the better tools. Though the screws are designed to accept metric hex heads (e.g the smaller screws have 1.5mm hex heads), however, I tend to find that torx bits work even better: T6 for the small screws and T8 for the larger screws. I believe this is because the torx bit tends to "bite" the screw head better, whereas hex heads that size can accidentally get rounded out if too much torque is applied.

## Disassembling the Case

It is necessary to disassemble the case to install stabilizers, and it is easier to test all the switch locations from the bottom of the PCB (which is exposed when the case is disassembled). To disassemble the case, remove the hex keys (a.k.a. Allen wrenches) from the small hardware bag that was in your EVO70 box. If you have a high profile case, you will want to use a larger hex key to remove the eight case screws from the bottom of EVO70, around the perimeter. If you have a low profile case, you will instead want to use a smaller hex key to remove the eight screws on the bottom that are not around the perimeter.

For a low proflie case, you generally don't need to unscrew the eight bigger screws around the edge of the case. These are used with standoffs to help ensure proper alignment of the mid piece. Unfortunately, because the counterdrill hole depth has some tolerance, in some cases, it becomes impossible to tighten the screw and standoff such that they are secure against the base piece (i.e. can't move). This can cause a bit of a rattle sound, but it can also be easily fixed: remove the offending standoff and add a small later of material between where the standoff goes and the base piece. For example, you could use the washers that may have been provided with your stabilizers. Or you could use a small bit of paper or tape--something to act like a washer and thicken the material between the screw head and the standoff.

At this point, the base should be removed, and the bottom of the PCB is exposed. Then use a hex key to remove the encoder knob by slightly loosening the set screw visible from the side of the encoder knob (do not remove the set screw; it is unnecessary to remove it, and the set screw is very easy to lose). Then (for both high and low profile kits) use a small hex key to remove the eight small screws from the top side (these screws are found amongst the switch locations and near the encoder). At this point, you should have a completely exposed PCB (with standoffs attached to it). The standoffs can remain attached to the PCB.
